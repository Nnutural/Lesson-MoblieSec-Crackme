...
EMU -avd AVD_API10_SMS_TEST  -no-boot-anim &
...
可以装原始包，但注意：**完全原始的 `crackme.apk` 在模拟器上大概率会先卡 `Challenge #1/#2`**，因为它检查 Wi-Fi MAC 和 IMEI。你现在能正常进界面，是因为我们已经绕过了环境检查。

**安装原始 APK**
在 VM 的 `~/Desktop/1/t3` 目录下执行：

```bash
ADB uninstall Com.zAWS.KeygenMe
ADB install -r crackme.apk
ADB shell am start -n Com.zAWS.KeygenMe/.main
```

如果看到 `Challenge #1`，说明原始包被模拟器检测拦住了，这是正常现象，不是安装失败。

**尝试不同密码**
每次测试前建议清数据：

```bash
ADB shell pm clear Com.zAWS.KeygenMe
ADB shell am start -n Com.zAWS.KeygenMe/.main
```
...
点 `Activate` 后输入，例如错误码：

```bash
ADB shell input text 12345678901
ADB shell input keyevent 66
ADB shell am force-stop Com.zAWS.KeygenMe
ADB shell am start -n Com.zAWS.KeygenMe/.main
```
...
如果是完全原始 APK + 真实设备，正确码会随真实 IMEI/MAC 变化，不是固定的。当前模拟器默认 IMEI 多半是 `000000000000000`，原始包会先判定模拟器异常，所以不适合作为“原始正确密码”验证环境。


对，这就是矛盾点：**完全原始 APK 在模拟器上看不到注册界面**，因为它先做 `Challenge #1/#2` 环境检测，检测到模拟器就退出。要对比“原始注册逻辑”的错误/正确密码状态，需要用我刚准备的对照包目录：

[crackme_envonly_decoded](D:/Nnutural/Desktop/BUPT_6/移动终端安全/final/Lesson-MoblieSec-Crackme/t3/crackme_envonly_decoded)

它只绕过模拟器环境检测，**没有绕过注册校验**。

在 VM 的 `~/Desktop/1/t3` 下构建它：

```bash
APKTOOL b crackme_envonly_decoded crackme_envonly_unsigned.apk
JARSIGNER -verbose -keystore ~/keys/softsec.keystore -storepass android -keypass android -sigalg MD5withRSA -digestalg SHA1 -signedjar crackme_envonly_signed.apk crackme_envonly_unsigned.apk softsec
...
19914427050
```
ADB shell input text 19914427050
...
输入后重启应用，仍显示锁就是原始逻辑失败状态；输入 `19914427050` 后重启，显示解锁就是原始逻辑成功状态。

我也把这套流程写进了 [reverse_steps_ubuntu12.md](D:/Nnutural/Desktop/BUPT_6/移动终端安全/final/Lesson-MoblieSec-Crackme/reverse_steps_ubuntu12.md:292)。
